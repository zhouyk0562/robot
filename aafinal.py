...
class TagTracker:
    def __init__(self):
        ...
        # 初始化两个摄像头
        self.cap1 = self._init_camera(0)  # 第一个摄像头
        self.cap2 = self._init_camera(1)  # 第二个摄像头
        ...
        # 多线程相关
        self.frame_queue1 = Queue(maxsize=1)  # 摄像头1的帧队列
        self.frame_queue2 = Queue(maxsize=1)  # 摄像头2的帧队列
        self.result_queue = Queue(maxsize=2)
        self.running = False

        # 启动检测线程
        self.detection_thread1 = threading.Thread(
            target=self._detection_worker,
            args=(self.frame_queue1,),
            daemon=True
        )
        self.detection_thread2 = threading.Thread(
            target=self._detection_worker,
            args=(self.frame_queue2,),
            daemon=True
        )
    ...
    def _init_camera(self, camera_index):
        ...
        for i in range(5):
            ...
            if cap.isOpened():
                ...
                return cap
            print(f"摄像头{camera_index}初始化失败，尝试 {i + 1}/5")
            time.sleep(1)
        print(f"无法初始化摄像头{camera_index}，请检查连接")
        exit(1)
    ...
    def _detection_worker(self, frame_queue):
        """检测线程工作函数"""
        while self.running:
            if not frame_queue.empty():
                ...
                # 没有检测到标签
                self.result_queue.put((False, 0, 0, None, 0))
    ...
    def track_tag(self):
        ...
        try:
            ...
            while True:
                ...
                # 非阻塞式放入队列
                if self.frame_queue1.empty():
                    self.frame_queue1.put(frame1.copy())
                if self.frame_queue2.empty():
                ...
                while not self.result_queue.empty():
                    found, distance, angle, tag, score = self.result_queue.get()
                    if found and score > best_score:
                        best_result = (distance, angle, tag)
                        best_score = score

                if best_result:
                    distance, angle, tag = best_result
                    print(
                        f"[FPS: {fps:.1f}] 追踪Tag {tag.tag_id} | 距离: {distance:.2f}m | 角度: {angle:.1f}° | 得分: {best_score:.2f}")
            ...
            self.running = False
            self.frame_queue1.put(None)  # 通知线程退出
            self.frame_queue2.put(None)  # 通知线程退出
            self.detection_thread1.join()
            self.detection_thread2.join()
            self.set_motor_speed(0, 0)
            self.cap1.release()
            self.cap2.release()
            print("程序安全退出")
